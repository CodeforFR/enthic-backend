...
def bdd_connexion():
    return create_engine(
        "mysql+pymysql://"
        + CONFIG["mySQL"]["enthic"]["user"]
        + ":"
        + CONFIG["mySQL"]["enthic"]["password"]
        + "@"
        + CONFIG["mySQL"]["enthic"]["host"]
        + "/enthic"
    )
...
def get_metadata(siren):
    select_request = select(AccountabilityMetadata).where(
        AccountabilityMetadata.siren == siren
    )
...
def sum_bundle_into_database(siren, declaration, accountability, bundle, amount):
    existing_bundle = SESSION.execute(
        select(Bundle).where(
            Bundle.siren == siren,
            Bundle.declaration == declaration,
            Bundle.accountability == accountability,
...
def replace_bundle_into_database(
    siren, declaration, accountability, bundle, amount, add_detail_mode
):
    existing_bundle = SESSION.execute(
        select(Bundle).where(
            Bundle.siren == siren,
    ...
    if len(existing_bundle) == 1:
        if add_detail_mode and abs(existing_bundle[0][0].amount - float(amount)) > 10:
            print(
                "Différences entre bundles alors que ça devrait être pareil",
                existing_bundle[0][0].amount,
                amount,
            )
        existing_bundle[0][0].amount = float(amount)
        SESSION.flush()
...
def replace_metadata_ORM(metadata, metadata_to_replace):
    existing_metadata_list = SESSION.execute(
        select(AccountabilityMetadata).where(
            AccountabilityMetadata.siren == metadata.siren,
            AccountabilityMetadata.declaration == metadata.declaration,
            AccountabilityMetadata.accountability == metadata.accountability,
            AccountabilityMetadata.date_cloture_exercice == metadata_to_replace[3],
        )
    ).fetchall()
