...
with open(CONFIG["accountOntologyCSV"]) as infile:
...
def get_siren_data_from_insee_api(siren):
    ...
    if response.status_code == 401:
        print(
            "Le token d'accès à l'API INSEE n'est plus valide, veuillez en générer un nouveau pour le mettre dans le fichier de configuration, à partir du site web https://api.insee.fr/catalogue/site/themes/wso2/subthemes/insee/pages/applications.jag"
        )
        exit()
    ...
    if (
        ...
        == "Unité légale non diffusable (" + str(siren) + ")"
    ):
        return None
...
def get_siret_data_from_insee_api(siren):
    ...
    postal_code = content["etablissements"][0]["adresseEtablissement"][
        "codePostalEtablissement"
    ]
    town = content["etablissements"][0]["adresseEtablissement"][
